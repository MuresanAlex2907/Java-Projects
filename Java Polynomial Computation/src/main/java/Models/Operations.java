...
public class Operations {
	...
	public int[] adunare(Polynomial nr1, Polynomial nr2) {
		...
		nr1.polynom.forEach(p1->{ 
		    a[p1.p]=p1.c;//at index i we add the coefficient corresponding with i th power in p1
		});
		nr2.polynom.forEach(p2->{ 
		    a[p2.p]=p2.c+a[p2.p];// we add to the i th index the coefficient corresponding with i th power in p2
		});
...
//		}
	...
	public void afisare(View view, int[] result) {
		...
		if (ok == 1) {
			...
			s = "0";
		view.result.setText(s);//set the result label
	}
